...
void solve(int t)
{
	...
	if (n % 2 == 1 and m % 2 == 1)
	{
		if (a[n - 1][m - 1] == 1)
		{
			ans.pb({{n - 1, m - 1}, {n - 2, m - 1}, {n - 1, m - 2}});
			for (auto j : ans.back())
			{
				a[j.F][j.S] ^= 1;
			}
	...
	//if (m % 2 == 1 and n % 2 == 1) while (!foo(n - 2, m - 2));
